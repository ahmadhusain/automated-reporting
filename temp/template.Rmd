...
ggplot(data_viz, aes(x = education_field, y = n)) +
  geom_col(aes(fill = n)) +
  geom_text(aes(y = n + max(n) * 0.075, label = label)) +
...
Our plot is a very prominent example for that: we could already see which education_field is the highest or the lowest, and we could also see the big picture regarding the ranking in terms of number of employee.

For example, let’s try the same visualization, but now using job_role as our categorical variable:
...
ggplot(data_viz, aes(x = job_role, y = n)) +
  geom_col(aes(fill = n)) +
  geom_text(aes(y = n + max(n) * 0.125, label = label)) +
  scale_y_continuous(expand = expand_scale(mult = c(0, 0.1))) +
  guides(fill = FALSE) +
  coord_flip() +
  labs(
...
# visualize
ggplot(data_viz, aes(x = job_role, y = n)) +
  geom_col(aes(fill = attrition), position = "stack") +
  scale_y_continuous(expand = expand_scale(mult = c(0, 0.1))) +
  coord_flip() +
  labs(
    title = "Number of employee",
...
This bar plot variation is called stacked bar plot. It help us to see the ranking in general, while also see the share of some more categorical variable inside each levels. For example, even though we still see the original ranking as usual, now we gain the insight that Sales Representative department have the highest proportion of attrition relative to the number of employee inside that department. A very crucial finding for this context.

## Numerical Value

```{r}
ggplot(data_attrition, aes(x = monthly_income)) +
  geom_histogram(fill = "red",bins = 30, colour = "black") +
  scale_x_continuous(
...
For example, let’s try to visualize how years at company relate to the monthly income for laboratory technicians samples.
